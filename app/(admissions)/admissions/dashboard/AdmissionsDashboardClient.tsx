  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const dd = obj ? (obj['deadline_display'] as string | undefined) : undefined;
        const d = obj ? (obj['deadline'] as string | undefined) : undefined;
        if (appId) setApplicationId(appId);
        if (dd) {
          setDeadlineDisplay(dd);
        } else if (d) {
          try {
            const parsed = new Date(d);
            if (!isNaN(parsed.getTime())) {
              setDeadlineDisplay(parsed.toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric' }));
            }
          } catch {
